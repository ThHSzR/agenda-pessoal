...
function initTables() {
  ...
  // ── migração: agendamentos legados → agendamento_procedimentos
  const semProcs = db.prepare(`
    SELECT a.id, a.procedimento_id, a.variante_id, a.valor_cobrado
    FROM agendamentos a
    WHERE a.procedimento_id IS NOT NULL
      AND NOT EXISTS (
        SELECT 1 FROM agendamento_procedimentos ap WHERE ap.agendamento_id = a.id
      )
  `).all();
  if (semProcs.length > 0) {
    ...
    const migrar = db.transaction(() => {
      semProcs.forEach(a => {
        const row = getDur.get(a.variante_id || null, a.procedimento_id);
        insAP.run(a.id, a.procedimento_id, a.variante_id || null, a.valor_cobrado || 0, row ? row.dur : 0);
      });
    });
    migrar();
  }
